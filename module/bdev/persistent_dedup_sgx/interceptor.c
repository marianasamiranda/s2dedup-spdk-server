...
uint64_t write_block(uint64_t offset, char * content) {
	...
	if (content ==NULL){ 
		...
		sleep(5);
      	     ...
      	     compute_hash(digest, content, offset * BLOCKLEN);
	}
...
void deal_db(){
	...
	destroy_leveldb("module/bdev/persistent_dedup_sgx/dbs/metadata");
	destroy_leveldb("module/bdev/persistent_dedup_sgx/dbs/index");
	printf("DBs Destroyed\n");
	printf("Creating DBs\n");
	create_leveldb_metadata("module/bdev/persistent_dedup_sgx/dbs/metadata");
	create_leveldb_index("module/bdev/persistent_dedup_sgx/dbs/index");
	printf("DBs created\n");
	sem_post(&mutex);
}
...
void init_dedup(unsigned long data_size, unsigned long metadata_size, unsigned long blocklen){
	fp = fopen("module/bdev/persistent_dedup_sgx/dedup_ratio.res","w+");
